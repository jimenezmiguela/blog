...
RSpec.describe "Orders", type: :request do
  ...
  describe "post orders_path with valid data" do
    it "saves a new entry and redirect to the show path for the entry" do
  ...
  describe "post orders_path with invalid data" do
    it "does no save a new entry or redirect" do
      order_attributes = FactoryBot.attributes_for(:order)
  ...
  describe "put orders_path with valid" do
  ...
  describe "put orders_path with invalid data" do
    it "does not update the customer record or redirect" do
      order = FactoryBot.create(:order)
      put order_path(id: order.id), params: {order: {customer_id: 5555}} # a non-existent customers
      order.reload
